...
coalesce(a.brand,l.brand) as brand,
coalesce(a.account_key,l.account_key) as account_key,
coalesce(a.region,l.amazon_region_id) as region,
coalesce(a.marketplace_key,l.marketplace_key) as marketplace_key,
coalesce(a.date_day, l.posted_local_date) as date_day,
coalesce(a.channel_product_id,l.asin) as channel_product_id,
coalesce(a.sku,l.sku) as sku,
coalesce(a.currency,l.currency) as currency,
a.color,
a.rate_to_usd,
a.region_name,
a.internal_sku_category,
...
a.DIST_OTHER_AMOUNT,
l.other_amount as other_amount,
a.RESTOCKING_FEE,
l.restocking_fee as ledger_restocking_fee,
a.brandhut_commission,
a.turner_costs,
a.freight,
a.product_samples,
a.miscellaneous_cost,
a.ad_spend_manual
from {{ref('product_pl_daily')}} a 
left join {{ref('finance_pl_pivot_new')}} l
    on a.account_key = l.account_key
    and a.region = l.amazon_region_id
    and a.marketplace_key = l.marketplace_key
    and a.date_day = l.posted_local_date
    and a.channel_product_id = l.asin
    and a.SKU = l.sku
...
sum(CAST(other_amount AS NUMERIC(18,2))) AS other_amount,
